...
pub enum ConnectError {
    ...
    #[error("bucket not configured - set S3_BUCKET env var or add to .buquet.toml")]
    MissingBucket,
    /// Region not configured in env vars or config file.
    #[error("region not configured - set S3_REGION env var or add to .buquet.toml")]
    MissingRegion,
    /// S3 storage error.
    #[error("S3 error: {0}")]
...
/// 3. Config file (.buquet.toml)
///
/// # Example
///
/// ```no_run
/// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
/// use buquet::queue::{connect, ConnectOptions};
///
...
impl Queue {
    ...
    pub async fn submit(
        ...
        counter!("buquet.tasks.submitted", "task_type" => task.task_type.clone()).increment(1);
    ...
    pub async fn cancel_with_reason(
        ...
        counter!("buquet.tasks.cancelled", "task_type" => updated.task_type.clone()).increment(1);
    ...
    pub async fn mark_expired(&self, task_id: Uuid) -> Result<Task, StorageError> {
        ...
        counter!("buquet.tasks.expired", "task_type" => updated.task_type.clone()).increment(1);
