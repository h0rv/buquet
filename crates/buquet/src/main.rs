...
async fn run_doctor(
    ...
    // Check S3_BUCKET
    let bucket = match buquet_config.as_ref().and_then(|c| {
    ...
    // Check S3_REGION
    let region = match buquet_config.as_ref().and_then(|c| {
    ...
    if allow_no_versioning() {
        println!(
            "\u{26A0} Bucket '{}' versioning check skipped (BUQUET_ALLOW_NO_VERSIONING)",
            bucket
        );
    } else {
        ...
        Commands::MigrateShards {
            ...
            if dry_run {
                ...
                if json {
                    ...
                    println!(
                        "  buquet migrate-shards --from {} --to {} --execute",
                        from, to
                    );
                }
            } else {
