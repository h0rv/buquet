...
**Worker config**
```python
worker = buquet.Worker(queue, "worker-1", shards)
...
buquet concurrency status [GROUP]

# Manually release stuck slot (admin)
buquet concurrency release GROUP SLOT_ID --force

# Set/update limit
buquet concurrency set GROUP --limit N
```
...
Metrics:
- `buquet_concurrency_slots_active{group}` - gauge
- `buquet_concurrency_slots_limit{group}` - gauge
- `buquet_concurrency_acquisition_total{group,result=acquired|rejected}` - counter
- `buquet_concurrency_wait_seconds{group}` - histogram (time until slot acquired, for queued tasks)
...
1. Add `buquet concurrency` subcommand
2. Add metrics
