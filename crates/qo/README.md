...
```rust
let queue = qo::connect(Config::from_env()?).await?;
...
- **Cron schedules**: `qo schedule create daily -t report -c "0 9 * * *"`
- **Cancellation**: `qo cancel <id>` or `queue.cancel(id)`
- **TTL/Expiration**: `--ttl 1h` or `--expires-at "..."`
- **Retries**: Exponential backoff with configurable policy

## Configuration

Create `.qo.toml` in your project root (auto-discovered):
