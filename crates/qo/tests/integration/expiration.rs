...
use qo::worker::{claim_task, ClaimResult, IndexSweeper, SweepMode, SweeperConfig};
use serde_json::json;
...
async fn test_mark_expired_fails_on_completed() {
    ...
    // Complete the task
    qo::worker::complete_task(&queue, claimed_task.id, lease_id, json!({"done": true}))
        .await
...
async fn test_expiration_does_not_affect_running_tasks() {
    ...
    let completed = qo::worker::complete_task(
        &queue,
        claimed_task.id,
        lease_id,
