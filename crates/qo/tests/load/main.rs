...
//!
//! ```bash
//! source crates/qo/examples/.env.example
//! cargo test -p qo --test load --release --features load-tests -- --nocapture
//! ```
...
use qo::queue::Queue;
use qo::storage::{S3Client, S3Config};
use std::sync::Arc;
...
/// - `S3_BUCKET`: Bucket name (default: `qo-dev`)
...
async fn test_queue() -> Queue {
    ...
    let client = S3Client::new(config).await.expect(
        "Failed to create S3 client - is LocalStack running? Try: just up && source crates/qo/examples/.env.example",
    );
    Queue::new(client)
}
...
async fn test_high_contention() {
    ...
    println!(
        "Claim conflicts (expected due to high contention): {}",
        conflicts
    );
