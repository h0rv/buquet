...
pub async fn execute_task(task: &Task, handlers: &HandlerRegistry) -> ExecutionResult {
    ...
    let Some(handler) = handlers.get(&task.task_type) else {
        counter!("qo.tasks.failed", "task_type" => task.task_type.clone(), "reason" => "no_handler")
            .increment(1);
    ...
    // Record metrics
    histogram!("qo.task.duration_seconds", "task_type" => task_type.clone())
        .record(duration.as_secs_f64());

    match &result {
        ExecutionResult::Success(_) => {
            counter!("qo.tasks.completed", "task_type" => task_type).increment(1);
        }
        ExecutionResult::Failed(TaskError::Timeout) => {
            counter!("qo.tasks.failed", "task_type" => task_type, "reason" => "timeout")
                .increment(1);
        }
        ExecutionResult::Failed(TaskError::Retryable(_)) => {
            counter!("qo.tasks.failed", "task_type" => task_type, "reason" => "retryable")
                .increment(1);
        }
        ExecutionResult::Failed(TaskError::Permanent(_)) => {
            counter!("qo.tasks.failed", "task_type" => task_type, "reason" => "permanent")
                .increment(1);
        }
        ExecutionResult::Failed(TaskError::Reschedule(delay)) => {
            counter!("qo.tasks.rescheduled", "task_type" => task_type).increment(1);
            #[allow(clippy::cast_precision_loss)]
            let delay_f64 = *delay as f64;
            histogram!("qo.reschedule.delay_seconds", "task_type" => task.task_type.clone())
                .record(delay_f64);
        }
    }
