//! Python bindings for the qo task queue.
//!
...
/// The qo Python module.
///
/// Provides access to the S3-based task queue from Python.
#[pymodule]
#[pyo3(name = "_qo")]
pub fn qo(m: &Bound<'_, PyModule>) -> PyResult<()> {
...
/// - `QO_SHARD_PREFIX_LEN` (optional): Number of hex characters for shard prefix (1-4)
///
...
/// * `shard_prefix_len` - Optional shard prefix length 1-4 (overrides `QO_SHARD_PREFIX_LEN`)
///   - 1 = 16 shards (default)
///   - 2 = 256 shards
///   - 3 = 4096 shards
///   - 4 = 65536 shards
