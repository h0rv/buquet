...
//! - Connecting to qo via S3 (auto-loads .qo.toml config)
...
async fn main() -> anyhow::Result<()> {
    ...
    // Connect using automatic config resolution (env vars / .qo.toml)
    ...
    let runner_config = RunnerConfig {
        polling: qo::worker::PollingStrategy::fixed(500),
