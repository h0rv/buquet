...
use oq::storage::{PutCondition, S3Client};
use std::sync::Arc;
...
impl StateManager {
    ...
    pub async fn get(&self, wf_id: &str) -> Result<(WorkflowState, String)> {
        ...
        let (data, etag) = self.client.get_object(&key).await.map_err(|e| {
            if matches!(e, oq::storage::StorageError::NotFound { .. }) {
    ...
    pub async fn update(
        ...
        let new_etag = self
            ...
            .await
            .map_err(|e| {
                if matches!(e, oq::storage::StorageError::PreconditionFailed { .. }) {
    ...
    pub async fn create_if_not_exists(
            ...
            .await
        {
            ...
            Err(oq::storage::StorageError::PreconditionFailed { .. }) => Ok(None),
    ...
    pub async fn get_step_result(
        ...
        match self.client.get_object(&key).await {
            ...
            Err(oq::storage::StorageError::NotFound { .. }) => Ok(None),
            Err(e) => Err(WorkflowError::Storage(e)),
        }
    }
}
