...
use oq_workflows::state::StateManager;
use oq_workflows::sweeper::WorkflowSweeper;
use tracing::info;
use tracing_subscriber::{fmt, EnvFilter};

/// oq-workflow - Workflow orchestration CLI
#[derive(Parser, Debug)]
#[command(
    name = "oq-workflow",
    version,
...
async fn main() -> anyhow::Result<()> {
    // Initialize logging
    fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("oq=info".parse()?))
        .init();
