...
use crate::types::{OnFailure, StepDef, StepStatus, WorkflowState};
...
mod tests {
    ...
    fn test_has_unrecoverable_failure_fail_workflow() {
        ...
        if let Some(step) = steps.get_mut("a") {
            step.on_failure = OnFailure::FailWorkflow;
        } else {
            assert!(false, "expected step a");
            return;
        }
    ...
    fn test_get_compensation_chain_with_handlers() {
        ...
        if let Some(step) = steps.get_mut("b") {
            step.has_compensation = true;
        } else {
            assert!(false, "expected step b");
            return;
        }
    ...
    fn test_topological_sort_empty() {
        ...
        let Ok(result) = topological_sort(&steps) else {
            assert!(false, "expected topological sort success");
            return;
        };
    ...
    fn test_topological_sort_cycle_two_nodes() {
        ...
        let result = topological_sort(&steps);
        match result {
            Err(DagError::CycleDetected { steps }) => {
                assert!(steps.contains(&"a".to_string()));
                assert!(steps.contains(&"b".to_string()));
            }
            _ => {
                assert!(false, "expected CycleDetected error");
                return;
            }
        }
    }
