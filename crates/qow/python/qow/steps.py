...
logger = logging.getLogger("qow.steps")

if TYPE_CHECKING:
    from collections.abc import Callable, Coroutine

    from qo import Queue, TaskContext
...
async def handle_step_failure(
    ...
    logger.error("Workflow %s step '%s' failed: %s", wf_id, step_name, error)
...
async def handle_step_cancellation(
    ...
    logger.warning("Workflow %s step '%s' cancelled: %s", wf_id, step_name, reason)
