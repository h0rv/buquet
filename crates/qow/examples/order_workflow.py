#!/usr/bin/env python3
"""
Order Fulfillment Workflow - showcasing qow features.
...
if TYPE_CHECKING:
    from qow.workflow import StepContext
...
async def run_worker() -> None:
    ...
    print(f"\n{BOLD}qow worker{RESET} - Order Fulfillment\n")
    log_tree()

    queue = await qo.connect()
    worker = qo.Worker(queue, "order-worker", queue.all_shards())
...
def main() -> None:
    if len(sys.argv) < 2:
        print(f"""
{BOLD}qow Order Workflow Example{RESET}
