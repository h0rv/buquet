...
use qow::state::StateManager;
use qow::sweeper::WorkflowSweeper;
use tracing::info;
use tracing_subscriber::{fmt, EnvFilter};

/// qo-workflow - Workflow orchestration CLI
#[derive(Parser, Debug)]
#[command(
    name = "qo-workflow",
    version,
...
async fn main() -> anyhow::Result<()> {
    // Initialize logging
    fmt()
        .with_env_filter(EnvFilter::from_default_env().add_directive("qo=info".parse()?))
        .init();
