...
import qo
from qow import StepContext, Workflow


def pytest_configure(config: pytest.Config) -> None:
    """Register custom markers."""
    config.addinivalue_line(
        "markers", "integration: marks tests as integration tests (require S3)"
    )
...
def _get_s3_bucket() -> str:
    """Get S3 bucket (test default: qo-dev)."""
...
@pytest.fixture
async def queue() -> qo.Queue:  # noqa: ANN201
    """Connect to qo queue (test default: LocalStack)."""
    return await qo.connect(
