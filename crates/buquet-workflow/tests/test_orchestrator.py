...
from buquet_workflow.orchestrator import (
...
class TestReconcileRunningSteps:
    ...
    async def test_failed_task_triggers_step_failure(self):
        ...
        with (
            patch(
                "buquet_workflow.orchestrator.handle_step_failure",
                side_effect=mock_handle_failure,
            ),
            patch(
                "buquet_workflow.orchestrator.get_workflow_state",
    ...
    async def test_cancelled_task_triggers_step_cancellation(self):
        ...
        with (
            patch(
                "buquet_workflow.orchestrator.handle_step_cancellation",
    ...
    async def test_output_preserved_as_is(self):
        ...
        with (
            patch("buquet_workflow.orchestrator.get_engine", side_effect=mock_get_engine),
            patch(
                "buquet_workflow.orchestrator.get_workflow_state",
                side_effect=mock_get_state,
            ),
...
class TestNoStepsAfterFailure:
    ...
    async def test_no_steps_scheduled_after_failure(self):
        ...
        with patch(
            "buquet_workflow.orchestrator.update_workflow_state",
