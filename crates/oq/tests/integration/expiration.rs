...
use oq::worker::{claim_task, ClaimResult, IndexSweeper, SweepMode, SweeperConfig};
use serde_json::json;
...
async fn test_submit_with_ttl_seconds() {
    ...
    let expected_min = before_submit + Duration::seconds(ttl_seconds as i64);
    let expected_max = after_submit + Duration::seconds(ttl_seconds as i64) + Duration::seconds(1);

    assert!(
        expires_at >= expected_min && expires_at <= expected_max,
        "expires_at ({}) should be approximately now + ttl_seconds ({} to {})",
        expires_at,
        expected_min,
        expected_max
    );
...
async fn test_submit_with_explicit_expires_at() {
    ...
    assert!(
        diff <= 1,
        "Stored expires_at ({}) should match provided value ({})",
        stored_expires,
        explicit_expiry
    );
}
...
async fn test_mark_expired_fails_on_completed() {
    ...
    // Complete the task
    oq::worker::complete_task(&queue, claimed_task.id, lease_id, json!({"done": true}))
        .await
...
async fn test_ttl_zero_expires_immediately() {
    ...
    assert!(
        ...
        "Task with ttl_seconds=0 should expire immediately, expires_at={}, now={}",
        expires_at,
        now
    );
    ...
    match result {
        ...
        | ClaimResult::NotFound
        | ClaimResult::Expired => {
            // Expected - immediately expired task should not be claimable
        }
        ClaimResult::Claimed { .. } => {
            // This is also acceptable if the claim happens within the same second
            // The important thing is the expiration behavior
        }
    }
}
...
async fn test_expiration_does_not_affect_running_tasks() {
    ...
    let completed = oq::worker::complete_task(
        &queue,
        claimed_task.id,
        lease_id,
