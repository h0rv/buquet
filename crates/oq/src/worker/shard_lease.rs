...
impl ShardLeaseConfig {
    ...
    /// - `OQ_SHARD_LEASING`: Set to "1" or "true" to enable
    /// - `OQ_SHARD_LEASING_SHARDS_PER_WORKER`: Number of shards per worker (default: 16)
    /// - `OQ_SHARD_LEASING_TTL_SECS`: Lease TTL in seconds (default: 30)
    /// - `OQ_SHARD_LEASING_RENEW_SECS`: Renewal interval in seconds (default: 10)
    #[must_use]
    pub fn from_env() -> Self {
        let enabled = std::env::var("OQ_SHARD_LEASING")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);

        let shards_per_worker = std::env::var("OQ_SHARD_LEASING_SHARDS_PER_WORKER")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(16);

        let lease_ttl_secs = std::env::var("OQ_SHARD_LEASING_TTL_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(30);

        let renewal_interval_secs = std::env::var("OQ_SHARD_LEASING_RENEW_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(10);
