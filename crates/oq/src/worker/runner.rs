...
use super::{
    complete_task, execute_task, fail_task, reschedule_task, retry_task, try_claim_for_handler,
    ClaimResult, ExecutionResult, PollingStrategy, ShardLeaseConfig, ShardLeaseManager, Worker,
};
        ...
        if client.ensure_bucket_versioning_enabled().await.is_ok() {
            tracing::info!(
                ...
                "S3 bucket versioning is enabled. For production use, configure S3 lifecycle \
                 ...
                 See: https://docs.rs/oq/latest/guides/version-history-retention"
            );
        }
        ...
        loop {
            ...
            match self.poll_once_for_shards(&config, &shards_to_poll).await {
                Ok(Some((task, etag, lease_id))) => {
                    ...
                    // Execute the task
                    let result = execute_task(&task, self.handlers()).await;

                    // Handle the result
                    self.handle_execution_result(&task, &etag, lease_id, result)
                        .await;
