...
impl PyTaskContext {
    ...
    /// Long-running handlers should check this periodically.
...
",
    ...
    /// ```
    fn on_startup<'py>(slf: Py<Self>, py: Python<'py>, callback: PyObject) -> PyResult<PyObject> {
    ...
    /// ```
    fn on_shutdown<'py>(slf: Py<Self>, py: Python<'py>, callback: PyObject) -> PyResult<PyObject> {
        let hooks = slf.borrow(py).hooks.clone();
        let callback_clone = callback.clone_ref(py);
        pyo3_async_runtimes::tokio::get_runtime().block_on(async move {
            let mut hooks = hooks.write().await;
    ...
    /// ```
    fn on_success<'py>(slf: Py<Self>, py: Python<'py>, callback: PyObject) -> PyResult<PyObject> {
        let hooks = slf.borrow(py).hooks.clone();
        let callback_clone = callback.clone_ref(py);
        pyo3_async_runtimes::tokio::get_runtime().block_on(async move {
            let mut hooks = hooks.write().await;
    ...
    /// ```
    fn on_error<'py>(slf: Py<Self>, py: Python<'py>, callback: PyObject) -> PyResult<PyObject> {
        let hooks = slf.borrow(py).hooks.clone();
        let callback_clone = callback.clone_ref(py);
        pyo3_async_runtimes::tokio::get_runtime().block_on(async move {
            let mut hooks = hooks.write().await;
...
async fn execute_task_with_handler_and_context(
    ...
    let handler = {
        ...
        handlers
            .get(&task_type)
            .map(|h| Python::with_gil(|py| h.callback.clone_ref(py)))
    };
    ...
    let result = Python::with_gil(|py| {
        ...
        Ok::<PyObject, TaskError>(coro)
    })?;

    // Check if it's a coroutine and get the future if so
    let (is_coro, future_opt) = Python::with_gil(|py| {
...
async fn call_hook(callback: &PyObject) -> Result<(), String> {
    // Call the callback
    let result = Python::with_gil(|py| {
        let coro = callback.call0(py).map_err(|e| e.to_string())?;
        Ok::<PyObject, String>(coro)
    })?;

    // Check if it's a coroutine and await it if so
    let future_opt = Python::with_gil(|py| {
        let result = result.bind(py);
        let asyncio = py
            .import("asyncio")
...
async fn call_success_hook(callback: &PyObject, task: &crate::models::Task) -> Result<(), String> {
