...
use crate::queue::{IdempotencyScope, Queue, QueueConfig, SubmitOptions, TaskSchema};
...
///
/// Use `oq.connect()` to create a new queue connection.
#[pyclass(name = "Queue")]
#[derive(Clone)]
pub struct PyQueue {
    inner: Arc<Queue>,
...
pub async fn connect_impl(
    ...
    let endpoint = endpoint.or_else(|| env::var("S3_ENDPOINT").ok());
    let bucket = bucket
        .or_else(|| env::var("S3_BUCKET").ok())
        .ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>(
                "S3_BUCKET environment variable or bucket parameter required",
            )
        })?;
    let region = region
        ...
        .ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>(
                "S3_REGION environment variable or region parameter required",
            )
        })?;

    // Read shard_prefix_len from environment if not provided
    let shard_prefix_len = shard_prefix_len.or_else(|| {
        env::var("OQ_SHARD_PREFIX_LEN")
            .ok()
    ...
    let allow_no_versioning = env::var("OQ_ALLOW_NO_VERSIONING")
    ...
    let queue = if let Some(prefix_len) = shard_prefix_len {
        let queue_config = QueueConfig {
            shard_prefix_len: prefix_len,
        };
        queue_config.validate().map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyValueError, _>(format!("Invalid queue config: {e}"))
        })?;
        Queue::with_config(client, queue_config)
    } else {
        Queue::new(client)
    };
...
impl PyQueue {
    ...
    ///
    /// ```python
    /// queue = await oq.connect(shard_prefix_len=2)
