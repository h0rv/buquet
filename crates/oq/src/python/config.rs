...
//!
//! This module provides Python wrappers for `OqConfig`, `WorkerConfig`, and `MonitorConfig`.

use pyo3::prelude::*;
use std::path::PathBuf;

use crate::config::{load_config, MonitorConfig, OqConfig, WorkerConfig};
...
impl From<OqConfig> for PyConfig {
    fn from(config: OqConfig) -> Self {
        Self { inner: config }
    }
}
...
/// 1. Environment variables (`S3_BUCKET`, `S3_ENDPOINT`, `S3_REGION`, `OQ_*`)
/// 2. Profile-specific settings (if profile specified)
/// 3. Project-level `.oq.toml`
/// 4. User-level `~/.config/oq/config.toml`
/// 5. Built-in defaults
///
/// # Arguments
///
...
///
/// # Load default config
/// config = oq.load_config()
///
/// # Load with a specific profile
/// config = oq.load_config(profile="dev")
///
