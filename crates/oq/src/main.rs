//! oq - S3-only distributed task queue
...
use oq::cli::{Cli, Commands, ConfigCommands, ScheduleCommands, SchemaCommands};
use oq::config::{
    format_config, format_config_paths, load_config, validate_config, ConfigSources, OqConfig,
};
use oq::migration::ShardMigration;
use oq::models::{Task, TaskStatus, WorkerInfo};
use oq::queue::{Queue, SubmitOptions, TaskSchema};
use oq::storage::{PutCondition, S3Client, S3Config, StorageError};
use oq::worker::{
...
fn display_error(err: &anyhow::Error) {
    ...
    if let Some(task_err) = err.downcast_ref::<oq::TaskOperationError>() {
...
/// Convert OqConfig to S3Config for client initialization.
fn oq_config_to_s3_config(config: &OqConfig) -> Result<S3Config> {
...
fn allow_no_versioning() -> bool {
    match env::var("OQ_ALLOW_NO_VERSIONING") {
...
async fn run_doctor(
    ...
    println!("oq doctor - checking environment\n");

    let mut all_passed = true;
    let mut suggestions: Vec<String> = Vec::new();

    // Try to load config first
    let oq_config = match load_config(profile, custom_config) {
        Ok(cfg) => {
    ...
    // Check S3_BUCKET
    let bucket = match oq_config.as_ref().and_then(|c| {
    ...
    // Check S3_REGION
    let region = match oq_config.as_ref().and_then(|c| {
    ...
    let endpoint = oq_config.as_ref().and_then(|c| c.endpoint.clone());
    ...
    if allow_no_versioning() {
        println!(
            "\u{26A0} Bucket '{}' versioning check skipped (OQ_ALLOW_NO_VERSIONING)",
            bucket
        );
    } else {
    ...
    if write_test {
        ...
        let test_data = b"oq doctor write test".to_vec();
...
async fn run_scheduler(
    queue: &Queue,
    manager: &oq::queue::ScheduleManager,
    interval_secs: u64,
) -> Result<()> {
    ...
    loop {
        ...
        let now = Utc::now();
        let report = oq::queue::run_scheduler_tick(queue, manager, now).await?;
...
async fn run() -> Result<()> {
    ...
    let oq_config = load_config(cli.profile.as_deref(), cli.config.as_ref())?;
    let s3_config = oq_config_to_s3_config(&oq_config)?;

    // Initialize S3 client
    let client = S3Client::new(s3_config).await?;
    if allow_no_versioning() {
        tracing::warn!(
            "Bucket versioning check skipped (OQ_ALLOW_NO_VERSIONING). History/audit guarantees are disabled."
        );
    } else {
    ...
    match cli.command {
        ...
        Commands::Status { task_id, json } => match queue.get(task_id).await? {
            ...
            None => {
                let err = oq::TaskOperationError::not_found(task_id);
        ...
        Commands::Replay { task_id } => {
            oq::worker::replay_task(&queue, task_id).await?;
            println!("Task {} replayed", task_id);
        }

        Commands::Archive { task_id } => {
            oq::worker::archive_task(&queue, task_id).await?;
        ...
        Commands::Worker {
            ...
            let shard_list: Vec<String> = shards
                ...
                .or_else(|| oq_config.worker.shards.clone())
            ...
            if with_monitor {
                ...
                tracing::warn!(
                    "Embedded monitor disabled. Ensure `oq monitor` is running to prevent task stalls."
                );
            }
        ...
        Commands::Serve { port } => {
            let app = oq::web::create_router(queue);
        ...
        Commands::Monitor {
            ...
            let config = MonitorConfig {
                ...
                sweep_page_size,
                max_tasks_per_scan: oq_config.monitor.max_tasks_per_scan.unwrap_or(100),
            };
        ...
        Commands::MigrateShards {
            ...
            if dry_run {
                ...
                if json {
                    ...
                    println!("  oq migrate-shards --from {} --to {} --execute", from, to);
                }
            } else {
                // Execute migration
                println!(
        ...
        Commands::Schedule { command } => {
            use oq::queue::{Schedule, ScheduleManager};
