...
impl Queue {
    ...
    pub async fn submit(
        ...
        counter!("oq.tasks.submitted", "task_type" => task.task_type.clone()).increment(1);
    ...
    pub async fn extend_lease(
        ...
        let new_lease_expires_at = now + ChronoDuration::seconds(additional_secs);
    ...
    pub async fn cancel_with_reason(
        ...
        counter!("oq.tasks.cancelled", "task_type" => updated.task_type.clone()).increment(1);
    ...
    pub async fn cancel_by_type(
        ...
        // Step 2: For each shard, list pending tasks
        let mut matching_task_ids = Vec::new();
        for shard in shards {
            let tasks = self.list(&shard, Some(TaskStatus::Pending), 10000).await?;
    ...
    pub async fn mark_expired(&self, task_id: Uuid) -> Result<Task, StorageError> {
        ...
        counter!("oq.tasks.expired", "task_type" => updated.task_type.clone()).increment(1);
