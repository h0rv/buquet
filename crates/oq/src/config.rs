//! Configuration file support for oq.
//!
...
//! 2. Environment variables (`S3_BUCKET`, `S3_ENDPOINT`, `S3_REGION`, `OQ_*`)
...
/// Main configuration structure for oq.
...
pub struct ConfigSources {
    /// User-level config path (~/.config/oq/config.toml)
    pub user_config: Option<PathBuf>,
    /// Whether user config exists
    pub user_config_exists: bool,
    /// Project-level config path (.oq.toml)
...
impl ConfigFile {
    ...
    #[must_use]
    pub fn resolve(&self, profile: Option<&str>) -> OqConfig {
        let mut config = OqConfig {
...
/// Get the user-level config path (~/.config/oq/config.toml).
#[must_use]
pub fn get_user_config_path() -> Option<PathBuf> {
    dirs::config_dir().map(|p| p.join("oq").join("config.toml"))
}

/// Get the project-level config path (.oq.toml in current directory).
#[must_use]
pub fn get_project_config_path() -> PathBuf {
    PathBuf::from(".oq.toml")
}
...
fn apply_env_overrides(config: &mut OqConfig) {
    ...
    // Queue configuration
    if let Ok(val) = env::var("OQ_SHARD_PREFIX_LEN") {
        if let Ok(len) = val.parse() {
            config.queue.shard_prefix_len = Some(len);
        }
    }

    // OQ_* environment variables for worker/monitor
    if let Ok(val) = env::var("OQ_POLL_INTERVAL_MS") {
        if let Ok(ms) = val.parse() {
            config.worker.poll_interval_ms = Some(ms);
        }
    }
    if let Ok(val) = env::var("OQ_INDEX_MODE") {
        config.worker.index_mode = Some(val);
    }
    if let Ok(val) = env::var("OQ_SHARDS") {
    ...
    if let Ok(val) = env::var("OQ_MONITOR_CHECK_INTERVAL") {
        if let Ok(secs) = val.parse() {
            config.monitor.check_interval_secs = Some(secs);
        }
    }
    if let Ok(val) = env::var("OQ_MONITOR_SWEEP_INTERVAL") {
        if let Ok(secs) = val.parse() {
            config.monitor.sweep_interval_secs = Some(secs);
        }
    }
    if let Ok(val) = env::var("OQ_MONITOR_MAX_TASKS_PER_SCAN") {
        if let Ok(count) = val.parse() {
            config.monitor.max_tasks_per_scan = Some(count);
        }
    }
    if let Ok(val) = env::var("OQ_SWEEPER_MAX_OBJECTS_PER_SWEEP") {
        if let Ok(count) = val.parse() {
            config.monitor.max_objects_per_sweep = Some(count);
        }
    }
    if let Ok(val) = env::var("OQ_SWEEPER_MODE") {
        config.monitor.sweep_mode = Some(val);
    }
}

/// Apply polling strategy environment variable overrides.
fn apply_polling_env_overrides(config: &mut OqConfig) {
    let strategy = env::var("OQ_POLLING_STRATEGY").ok();
    let min_interval_ms = env::var("OQ_POLLING_MIN_INTERVAL_MS")
        .ok()
        .and_then(|v| v.parse().ok());
    let max_interval_ms = env::var("OQ_POLLING_MAX_INTERVAL_MS")
        .ok()
        .and_then(|v| v.parse().ok());
    let backoff_multiplier = env::var("OQ_POLLING_BACKOFF_MULTIPLIER")
        .ok()
        .and_then(|v| v.parse().ok());
...
fn apply_shard_leasing_env_overrides(config: &mut OqConfig) {
    let enabled = env::var("OQ_SHARD_LEASING")
        .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
        .ok();
    let shards_per_worker = env::var("OQ_SHARD_LEASING_SHARDS_PER_WORKER")
        .ok()
        .and_then(|v| v.parse().ok());
    let lease_ttl_secs = env::var("OQ_SHARD_LEASING_TTL_SECS")
        .ok()
        .and_then(|v| v.parse().ok());
    let renewal_interval_secs = env::var("OQ_SHARD_LEASING_RENEW_SECS")
        .ok()
        .and_then(|v| v.parse().ok());
...
#[must_use]
pub fn validate_config(config: &OqConfig) -> Vec<String> {
...
pub fn format_config(config: &OqConfig, sources: &ConfigSources) -> String {
...
pub fn format_config_paths(sources: &ConfigSources) -> String {
    ...
    output.push_str("  2. Environment variables (S3_BUCKET, OQ_*, etc.)\n");
    output.push_str("  3. Custom config (if --config specified)\n");
    output.push_str("  4. Project config (.oq.toml)\n");
    output.push_str("  5. User config (~/.config/oq/config.toml)\n");
...
"#;
        ...
        assert_eq!(config.bucket, "oq-prod");
...
"#;
        ...
        assert_eq!(dev_config.bucket, "oq-dev");
        assert_eq!(
            dev_config.endpoint,
        ...
        assert_eq!(staging_config.bucket, "oq-staging");
...
"#;
        ...
        assert_eq!(default_config.bucket, "oq-prod");
    ...
    fn test_env_overrides_file() {
        ...
        // For now, we test that OqConfig can be constructed with env-provided values
        // that would override file-based values.
        let file_config = OqConfig {
    ...
    fn test_project_config_overrides_user() {
        // .oq.toml (project-level) overrides ~/.config/oq/config.toml (user-level)
...
"#;
    ...
    fn test_load_missing_config_uses_defaults() {
        ...
        // When no config file exists, OqConfig::default() should be used
        let config = OqConfig::default();
...
"#;
        ...
        // S3 settings
        assert_eq!(config.bucket, "oq-production");
        assert_eq!(config.region, "us-west-2");
