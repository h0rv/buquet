...
End-to-end examples demonstrating oq's polyglot task queue. Python and Rust producers/workers can freely interoperate—tasks submitted from Python can be processed by Rust workers, and vice versa.

## Setup

### 1. Start LocalStack (local S3)
...
Python commands below assume you are in `crates/oq` so `uv` uses the correct
`pyproject.toml`.
...
[worker] Connecting to bucket: oq-dev
...
```bash
cargo run -p oq --example producer
```
