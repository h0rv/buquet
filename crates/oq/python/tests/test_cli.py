...
def _run_oq_command(args: list[str]) -> subprocess.CompletedProcess[str] | None:
    """Run an oq CLI command, returning None if binary not found."""
    try:
...
def _run_cargo_oq_command(args: list[str]) -> subprocess.CompletedProcess[str]:
    """Run oq CLI via cargo run."""
    return subprocess.run(  # noqa: S603
...
def _get_help_or_skip(subcommand: list[str], feature_name: str) -> subprocess.CompletedProcess[str]:
    ...
    result = _run_oq_command([*subcommand, "--help"])

    if result is None:
        result = _run_cargo_oq_command([*subcommand, "--help"])
...
class TestCLITailCommand:
    """Test that the oq tail CLI command exists and parses arguments."""

    def test_oq_cli_has_tail_subcommand(self) -> None:
        """oq CLI should have a 'tail' subcommand."""
        _get_help_or_skip(["tail"], "tail command")

    def test_tail_accepts_task_type_filter(self) -> None:
        """oq tail should accept --task-type argument."""
    ...
    def test_tail_accepts_status_filter(self) -> None:
        """oq tail should accept --status argument."""
        result = _get_help_or_skip(["tail"], "tail command")
        assert "--status" in result.stdout

    def test_tail_accepts_json_flag(self) -> None:
        """oq tail should accept --json flag for JSON output."""
        result = _get_help_or_skip(["tail"], "tail command")
        assert "--json" in result.stdout

    def test_tail_accepts_limit_option(self) -> None:
        """oq tail should accept --limit argument."""
        result = _get_help_or_skip(["tail"], "tail command")
        assert "--limit" in result.stdout

    def test_tail_accepts_interval_option(self) -> None:
        """oq tail should accept --interval argument for poll interval."""
        result = _get_help_or_skip(["tail"], "tail command")
...
class TestCLITailCommandParsing:
    ...
    def test_tail_with_all_options(self) -> None:
        """oq tail should accept all options together."""
        result = _get_help_or_skip(["tail"], "tail command")
...
class TestCLIGeneral:
    ...
    def test_help_lists_available_commands(self) -> None:
        ...
        result = _run_oq_command(["--help"])

        if result is None:
            result = _run_cargo_oq_command(["--help"])
