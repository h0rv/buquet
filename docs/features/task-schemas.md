...
Schemas are just another S3 object. oq stores them, retrieves them, and validates against them. That's it.
...
```bash
# Store
oq schema publish send_email schema.json

# Retrieve
oq schema get send_email

# Validate
oq schema validate send_email --input '{"to": "x"}'
oq schema validate send_email --output '{"sent": true}'

# List
oq schema list

# Delete
oq schema delete send_email
```
...
oq follows the same philosophy as the rest of the system: **simple primitives, user handles coordination.**

| Concern | oq's job | User's job |
|---------|----------|------------|
...
- `crates/oq/src/queue/schema.rs` (new) - ~30 lines: publish, get, list, delete
- `crates/oq/src/queue/validate.rs` (new) - ~20 lines: validate against schema
- `crates/oq/src/python/queue.rs` - Add schema methods
- `crates/oq/src/cli/commands.rs` - Add `Schema` subcommand
- `crates/oq/python/oq/_oq.pyi` - Type stubs
