...
| Debuggable | `oq status` shows expiration, clear why task wasn't processed |
...
```bash
# Submit with absolute expiration
oq submit -t send_reminder -i '{}' --expires-at "2026-01-28T17:00:00Z"

# Submit with TTL
oq submit -t process_event -i '{}' --ttl 1h
oq submit -t quick_task -i '{}' --ttl 5m
oq submit -t batch_job -i '{}' --ttl 7d
...
```
oq_tasks_expired_total{task_type}
oq_task_ttl_seconds{task_type}  # Histogram of TTL values
oq_tasks_near_expiration{task_type}  # Gauge of tasks expiring soon
```
...
- `crates/oq/src/models/task.rs` - Add `Expired` status, `expires_at`, `expired_at` fields
- `crates/oq/src/queue/ops.rs` - Add `mark_expired()`, TTL handling in submit
- `crates/oq/src/queue/submit.rs` - Add `expires_at`, `ttl` to `SubmitOptions`
- `crates/oq/src/worker/runner.rs` - Check expiration before claim
- `crates/oq/src/worker/monitor.rs` - Sweep for expired tasks
- `crates/oq/src/python/queue.rs` - Python bindings
- `crates/oq/src/cli/commands.rs` - Add `--expires-at`, `--ttl` flags
