...
| Debuggable | `oq status` shows pause state, clear operational status |
...
# Pause the queue
oq pause
oq pause --reason "Deploying new version"
oq pause --reason "Maintenance" --paused-by "alice"

# Pause with auto-resume
oq pause --reason "Quick fix" --resume-after 30m
oq pause --reason "Maintenance window" --resume-after 2h
oq pause --reason "Overnight batch" --resume-at "2026-01-29T06:00:00Z"
...
```bash
oq pause --reason "First pause"
oq pause --reason "Update reason"  # Error: Queue already paused
oq pause --reason "Update reason" --force  # Overwrites
```
...
oq pause --type send_email

# Pause specific shards
oq pause --shards 0,1,2,3
```
...
```
oq_queue_paused{bucket}  # Gauge: 1 if paused, 0 if not
oq_pause_duration_seconds{bucket}  # How long paused
oq_pause_events_total{bucket, action=pause|resume}
```
...
- `crates/oq/src/queue/control.rs` (new) - Pause/resume operations
- `crates/oq/src/models/control.rs` (new) - `PauseState` struct
- `crates/oq/src/worker/runner.rs` - Check pause before claiming
- `crates/oq/src/python/queue.rs` - Python bindings
- `crates/oq/src/cli/commands.rs` - Add `Pause`, `Resume` subcommands, update `Status`
