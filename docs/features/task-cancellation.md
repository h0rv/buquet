...
| Debuggable | `oq status` shows cancelled, history shows who cancelled |
...
```python
from oq import connect, TaskStatus
...
# Cancel with reason
oq cancel abc123 --reason "Duplicate submission"

# Cancel multiple tasks
oq cancel abc123 def456 ghi789

# Cancel all pending tasks of a type
oq cancel --type old_task_type

# Dry run (show what would be cancelled)
oq cancel --type old_task_type --dry-run
```
...
```
oq_tasks_cancelled_total{task_type}
oq_cancel_requests_total{task_type, outcome=success|already_cancelled|not_found|invalid_status}
```
...
- `crates/oq/src/models/task.rs` - Add `Cancelled` status, `cancel_requested`, `cancelled_at`, `cancelled_by`
- `crates/oq/src/queue/ops.rs` - Add `cancel()`, `cancel_many()`, `cancel_by_type()`
- `crates/oq/src/worker/runner.rs` - Check `cancel_requested` after handler returns
- `crates/oq/src/python/queue.rs` - Python bindings for cancel
- `crates/oq/src/python/worker.rs` - Add `context.is_cancellation_requested()`
- `crates/oq/src/cli/commands.rs` - Add `Cancel` subcommand
...
- CLI `oq cancel` command with:
  - Multiple task IDs: `oq cancel uuid1 uuid2 uuid3`
  - Cancel by type: `oq cancel --task-type old_task_type`
  - Reason tracking: `oq cancel uuid --reason "No longer needed"`
  - Metadata: `oq cancel uuid --cancelled-by "user:alice"`
  - JSON output: `oq cancel uuid --json`

## Non-Goals

- **Force kill running tasks**: No signals, cooperative only
- **Cancel with rollback**: oq doesn't know what to roll back
