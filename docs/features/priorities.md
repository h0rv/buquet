...
```python
from oq import connect, Priority
...
```bash
# High priority
oq submit -t send_alert -i '{"message":"Server down!"}' --priority 255
oq submit -t send_alert -i '{"message":"Server down!"}' --priority critical

# Normal priority (default)
oq submit -t send_email -i '{"to":"user@example.com"}'

# Low priority
oq submit -t cleanup -i '{}' --priority 50
oq submit -t cleanup -i '{}' --priority low

# Check task priority
oq get abc123 --json | jq .priority
```
