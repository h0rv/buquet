...
When something doesn't work, users waste time debugging. `oq doctor` immediately tells them what's wrong and how to fix it.
...
```
$ oq doctor
oq doctor - checking environment

✓ Environment variables set
✓ S3 client initialized
✓ Bucket 'oq-dev' exists and is accessible
...
async fn doctor(args: DoctorArgs) -> Result<()> {
    ...
    if args.write_test {
        let test_key = format!("_oq_doctor_test_{}", Uuid::new_v4());
