...
regular oq worker) checks schedules and submits tasks when due.
...
```bash
# One-shot scheduling
oq submit -t send_reminder -i '{"user_id":"123"}' --at "2026-01-28T09:00:00Z"
oq submit -t send_reminder -i '{"user_id":"123"}' --in 1h

# Create recurring schedule
oq schedule create daily-report \
...
oq schedule show daily-report

# Pause/resume
oq schedule pause daily-report
oq schedule resume daily-report

# Trigger immediately (outside schedule)
oq schedule trigger daily-report

# Delete
oq schedule delete daily-report
```
...
```bash
# Run scheduler as part of oq monitor
oq monitor --enable-scheduler

# Or as standalone
oq scheduler run
```

The scheduler uses the same CAS primitives as the rest of oq — multiple
...
- [Workflow-Lite](workflow-lite.md) — Schedules can trigger workflow starts
