...
```bash
OQ_POLLING_STRATEGY=adaptive
OQ_POLLING_MIN_INTERVAL_MS=100
OQ_POLLING_MAX_INTERVAL_MS=30000
OQ_POLLING_BACKOFF_MULTIPLIER=2.0
```
...
crates/oq/src/config.rs      # Config file parsing for polling settings
```
...
oq uses adaptive polling by default to minimize S3 costs:
