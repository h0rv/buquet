# Feature: oq-workflows (Durable Orchestration Layer)

## Status: Implemented (Separate Package)

**This is not part of core oq.** It is a **durable orchestration layer** built on oq's
existing primitives. Core oq remains a simple task queue.

Implementation lives in `crates/oq-workflows` (Rust + Python bindings).

## Overview

oq-workflows provides workflow orchestration without introducing a control plane,
external databases, or coordination services. It uses S3 as the only state store,
just like core oq.

## Comparison with Other Systems

Before diving in, let's be clear about what oq-workflows provides vs. alternatives:

| Aspect | Temporal | Hatchet | Prefect | oq-workflows |
|--------|----------|---------|---------|--------------|
...
**Sub-second latency**: oq is polling-based. Steps take 1-5 seconds to dispatch.
...
Each workflow step is a normal oq task. The orchestrator is a special task handler
...
Each step runs as an oq task with type `workflow.step:{workflow_type}:{step_name}`:
...
for workflow in list_workflows_by_status(ACTIVE_STATES):
    ...
    if orchestrator_task is None:
        ...
        if orchestrator_task.lease_expires_at < now:
            # Will be recovered by oq's timeout monitor
            pass
        else:
...
```python
# Run as part of oq monitor or standalone
...
Signals are **at-least-once**, consistent with oq's task execution model:

- The same signal may be sent multiple times (sender retries, network issues)
- Signal handlers **must be idempotent**
- oq does not deduplicate signals at the infrastructure level

This is intentional: oq doesn't solve idempotency for tasks, and doesn't solve
...
```python
from oq_workflows import Workflow, step
...
Use oq's `available_at` for durable timers:
...
Steps use oq's existing retry policies:
...
**oq cannot guarantee exactly-once effects.** It provides:
...
oq's built-in idempotency keys prevent duplicate step submissions:
...
Workflows and steps support cancellation (using oq's cancellation features):
...
### What oq-workflows Does NOT Guarantee
...
- **Exactly-once side effects**: Only achievable if downstream systems support
  idempotency AND your handler uses it correctly. oq provides idempotency keys
...
oq-workflows provides tools for idempotency, but **you must use them correctly**:
...
```python
from oq_workflows import Workflow, step, WorkflowClient
...
```bash
# Start a workflow
oq workflow start order_fulfillment '{"order_id": "ORD-123"}'

# Check status
oq workflow status wf-abc123

# List workflows
oq workflow list --type order_fulfillment --status running

# Cancel
oq workflow cancel wf-abc123 --reason "No longer needed"

# Send signal
oq workflow signal wf-abc123 approval '{"approved": true}'

# View step results
oq workflow steps wf-abc123
```
...
| Use Case | oq-workflows | Hatchet | Temporal |
|----------|--------------|---------|----------|
...
oq-workflows provides durable workflow orchestration for teams that want:
