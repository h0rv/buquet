...
oq list --task-type=send_email --status=failed

# All running order tasks in shard 'a'
oq list -s a --task-type=process_order --status=running

# Short form
oq list -t send_email
```
...
- `crates/oq/src/cli/commands.rs` - Add `task_type` field to `List` variant
- `crates/oq/src/main.rs` - Add filter logic in `Commands::List` handler
