# Getting Started with oq

oq is a distributed task queue built entirely on S3-compatible object storage. No databases. No message brokers. No coordination services. Just workers polling object storage and claiming tasks atomically.
...
Start the complete oq stack with a single command:
...
Install the oq Python SDK using uv:
...
async def main():
    queue = await connect(bucket="oq-dev", endpoint="http://localhost:4566")
...
```python
import asyncio
from oq import connect, Worker, RetryableError
...
Configure oq using environment variables:

| Variable | Description | Example |
|----------|-------------|---------|
| `S3_ENDPOINT` | S3-compatible endpoint URL | `http://localhost:4566` |
| `S3_BUCKET` | Bucket name for task storage | `oq-dev` |
...
oq distinguishes between two types of errors:
...
oq uses adaptive polling by default to minimize S3 costs. When tasks are available, polling is fast. When the queue is idle, polling backs off automatically.

```python
from oq import Worker, WorkerRunOptions, PollingStrategy
...
```rust
// Rust example
use oq::worker::{RunnerConfig, PollingStrategy};
...
```python
from oq import WorkerRunOptions, ShardLeaseConfig
...
oq uses S3 bucket versioning to maintain a complete audit trail of every task state transition. This enables powerful debugging with `oq history <task_id>` and compliance capabilities, but requires lifecycle policy configuration to manage storage costs at scale.
...
aws s3api put-bucket-lifecycle-configuration \
  --bucket your-oq-bucket \
  --lifecycle-configuration '{
    "Rules": [{
      "ID": "oq-version-retention",
      "Status": "Enabled",
      "Filter": {"Prefix": "tasks/"},
...
oq works with any S3-compatible storage that supports strong consistency and conditional writes. Tested providers include:
...
- Inspect task data directly with `aws s3 ls s3://oq/tasks/`
