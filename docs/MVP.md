...
OQ_INDEX_MODE=hybrid             # index-only | hybrid | log-scan
OQ_READY_PAGE_SIZE=100           # Page size for ready index listing
OQ_LOG_SCAN_PAGE_SIZE=50         # Page size for log scan listing
OQ_NO_MONITOR=false              # Disable embedded monitor alongside worker

# Monitor
OQ_MONITOR_CHECK_INTERVAL=30     # Seconds between timeout checks
OQ_MONITOR_WORKER_HEALTH_THRESHOLD=60
OQ_MONITOR_SWEEP_INTERVAL=300    # Seconds between index sweeps (0 disables)
OQ_MONITOR_SWEEP_PAGE_SIZE=1000

# Logging
RUST_LOG=info                   # trace/debug/info/warn/error

# Dev override (disables history guarantees)
OQ_ALLOW_NO_VERSIONING=1        # Skip bucket versioning check
```
...
```bash
# Submit a task
oq submit --type send_email --input '{"to": "user@example.com"}'
...
# List tasks
oq list [--shard 0-f] [--status pending|running|completed|failed|archived]
...
oq worker [--id worker-1] [--shards 0,1,2,3] [--index-mode hybrid] [--no-monitor]

# Run monitor (timeouts + index sweep)
oq monitor [--check-interval 30] [--worker-health-threshold 60] [--sweep-interval 300]
```
