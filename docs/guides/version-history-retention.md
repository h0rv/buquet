...
oq uses S3 bucket versioning to automatically maintain a complete audit trail of every task state transition. While this provides powerful debugging and compliance capabilities, it can also drive storage costs at scale without proper lifecycle management.
...
Every task in oq is stored as a single JSON object at `tasks/{shard}/{task_id}.json`. When S3 versioning is enabled, each state change creates a new version:
...
2. **LIST operations**: While oq uses index prefixes to avoid listing task objects directly, administrative queries and the dashboard may list versions.
...
A lifecycle policy for oq should:
...
```hcl
resource "aws_s3_bucket" "oq" {
  bucket = "your-oq-bucket"
}

resource "aws_s3_bucket_versioning" "oq" {
  bucket = aws_s3_bucket.oq.id
  versioning_configuration {
    status = "Enabled"
  }
}

resource "aws_s3_bucket_lifecycle_configuration" "oq" {
  bucket = aws_s3_bucket.oq.id
...
```hcl
resource "aws_s3_bucket_lifecycle_configuration" "oq_compliance" {
  bucket = aws_s3_bucket.oq.id

  rule {
    id     = "oq-version-retention-compliance"
    status = "Enabled"
...
aws cloudwatch get-metric-statistics \
  ...
  --dimensions Name=BucketName,Value=your-oq-bucket Name=StorageType,Value=StandardStorage \
