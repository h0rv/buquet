...
```python
import asyncio
from oq import connect, Worker, WorkerRunOptions, PollingStrategy

async def demo():
    queue = await connect(endpoint='http://localhost:4566', bucket='oq-dev')
...
Both the Rust worker (`crates/oq/src/worker/runner.rs`) and Python worker (`crates/oq/src/python/worker.rs`) had the same bug: they claimed tasks before checking if a handler was registered for the task type.

## Fix Applied

The fix centralizes the "check handler before claiming" logic in a new Rust function `try_claim_for_handler` in `crates/oq/src/worker/claim.rs`. This function is the **single source of truth** for this pattern, used by both Rust and Python workers.
...
```rust
// In crates/oq/src/worker/runner.rs
...
```rust
// In crates/oq/src/python/worker.rs
...
- `crates/oq/src/worker/claim.rs` - Added `try_claim_for_handler` function (source of truth)
- `crates/oq/src/worker/mod.rs` - Exported `try_claim_for_handler`
- `crates/oq/src/worker/runner.rs` - Rust worker now uses `try_claim_for_handler`
- `crates/oq/src/python/worker.rs` - Python worker now uses `try_claim_for_handler`

## Tests Added

- `crates/oq/python/tests/test_reschedule.py::TestWorkerHandlerFiltering` - Dedicated test class with 3 tests:
