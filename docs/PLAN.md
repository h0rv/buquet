...
- [ ] CLI structure using `clap` derive:
  ...
  Commands:
    ...
    worker   Run a worker
  ```
- [ ] `oq submit --type <TYPE> --input <JSON> [--timeout <SECS>] [--retries <N>]`
...
- [ ] `oq list [--shard <HEX>] [--status <STATUS>] [--limit <N>]`
...
**Integration Tests:**
- [ ] `crates/oq/tests/integration/submit_claim_complete.rs` - happy path
- [ ] `crates/oq/tests/integration/concurrent_claim.rs` - multiple workers race
- [ ] `crates/oq/tests/integration/retry_exhaustion.rs` - task fails until Failed status
- [ ] `crates/oq/tests/integration/timeout_recovery.rs` - monitor recovers stuck task
- [ ] `crates/oq/tests/integration/pagination_starvation.rs` - ensures paginated LIST makes progress
- [ ] `crates/oq/tests/integration/index_cleanup.rs` - ready/lease orphans are harmless
- [ ] `crates/oq/tests/integration/graceful_shutdown.rs` - no lost tasks
- [ ] `crates/oq/tests/integration/version_history.rs` - history shows all transitions
